...
socket.on('Send-kill-pokemon',(data)=> {
  maTran = removePokemon(
    data.toaDo1,data.toaDo2,
    maTran)
...
function chinh() {
  let point1: {x: number,y: number}, point2: {x: number,y: number};
  let ELMTarget_prev: HTMLElement;
  ...
  for (const pkm of listPkm54) {
    pkm.addEventListener("click", (e) => {
      let ELMTarget = <HTMLElement>e.target;
      let x = Number(ELMTarget.dataset.x),
        y = Number(ELMTarget.dataset.y);
