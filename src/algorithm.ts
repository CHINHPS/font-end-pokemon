export function createArrayPokemon(xRow: number, yColum: number) {
  // tạo ma trận full 0
...
export function createArrayZero(xRow: number, yColum: number){
...
export function checkLinePokemon(point1: {x: number,y: number}, point2: {x: number,y: number},dataPokemon){
...
function checkLineX(y1: number, y2: number, x: number, dataPokemon){
...
function checkLineY(x1: number, x2: number, y: number, dataPokemon){
...
function checkZ_X(point1: {x: number,y: number}, point2: {x: number,y: number}, dataPokemon){
  ...
  for (let x = point_min.x+1; x < point_max.x; x++) {
    if(
      checkLineY(point_min.x, x, point_min.y, dataPokemon) &&
...
function checkZ_Y(point1: {x: number,y: number}, point2: {x: number,y: number}, dataPokemon){
  ...
  for (let y = point_min.y+1; y < point_max.y; y++) {
    if(
      checkLineX(point_min.y,y, point_min.x, dataPokemon) &&
...
function checkUL_X(point1: {x: number,y: number}, point2: {x: number,y: number},type: number, dataPokemon){
  ...
  for (let x = startX; swapCondition(x,infinity_x,type); x += type) {
    ...
    if(
      checkLineY(point_min.x,  x,  point_min.y, dataPokemon) && // kiểm tra hàng ngang ở trên (1)
      checkLineY(point_max.x,  x,  point_max.y, dataPokemon) // kiểm tra hàng ngang dưới (2)
    ){
      if(
        checkLineX(point_min.y,  point_max.y - 1,  x,  dataPokemon) // check hàng dọc th(1) ăn được thì trả về kết quả
      ){
        if(dataPokemon[point_min.y][point_min.x] == dataPokemon[point_max.y][point_max.x]){
...
function checkUL_Y(point1: {x: number,y: number}, point2: {x: number,y: number}, type: number, dataPokemon){
  ...
  for (let y = point_max.y; swapCondition(y,infinity_y,type); y += type) {
    ...
    if(
      checkLineX(point_min.y,  y,  point_min.x, dataPokemon) && // kiểm tra hàng dọc (1)
